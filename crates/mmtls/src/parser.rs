use crate::Result;
use crate::consts::{MAGIC_ABORT, MAGIC_HANDSHAKE, MAGIC_RECORD, MAGIC_SYSTEM};
...
fn parse_handshake_payload(data: &[u8]) -> Vec<ParsedField> {
    if data.len() < 5 {
        return encrypted_fallback(data);
    }
    let total_len = u32::from_be_bytes([data[0], data[1], data[2], data[3]]);
    let flag = data[4];
    if total_len as usize != data.len() - 4 {
...
fn parse_client_hello_fields(data: &[u8]) -> Vec<ParsedField> {
    ...
    fields[0].children.push(ParsedField {
        name: "Total Length".into(),
        value: format!("{total_len}"),
        raw: vec![],
        children: vec![],
    });
    ...
    fields[0].children.push(ParsedField {
        name: "Flag".into(),
        value: format!("0x{:02X}", b[0]),
        raw: vec![],
        children: vec![],
    });

    let mut vb = [0u8; 2];
    if r.read_exact(&mut vb).is_err() {
        return encrypted_fallback(data);
    }
    let pv = u16::from_le_bytes(vb);
    fields[0].children.push(ParsedField {
        name: "Protocol Version".into(),
        value: format!("0x{pv:04X} (LE)"),
        raw: vec![],
        children: vec![],
    });
    ...
    let cs_count = b[0];
    let mut csf = ParsedField {
        name: "Cipher Suites".into(),
        value: format!("({cs_count})"),
        raw: vec![],
        children: vec![],
    };
    let mut csb = [0u8; 2];
    for i in 0..cs_count {
        if r.read_exact(&mut csb).is_err() {
            break;
        }
        let cs = u16::from_be_bytes(csb);
        csf.children.push(ParsedField {
            name: format!("[{i}]"),
            value: format!("{} (0x{cs:04X})", cipher_suite_name(cs)),
            raw: vec![],
            children: vec![],
        });
    }
    ...
    if r.read_exact(&mut random).is_err() {
        return encrypted_fallback(data);
    }
    fields[0].children.push(ParsedField {
        name: "Client Random".into(),
        value: format!("{} (32 bytes)", hex::encode(random)),
        raw: random.to_vec(),
        children: vec![],
    });
    ...
    fields[0].children.push(ParsedField {
        name: "Timestamp".into(),
        value: format!("{ts} ({})", chrono_now(ts)),
        raw: vec![],
        children: vec![],
    });

    let mut el_buf = [0u8; 4];
    if r.read_exact(&mut el_buf).is_err() {
        return fields;
    }
    ...
    let mut extf = ParsedField {
        name: "Extensions".into(),
        value: format!("({ext_count}), total {ext_len} bytes"),
        raw: vec![],
        children: vec![],
    };
    ...
    for _ in 0..ext_count {
        if let Some(e) = parse_extension(&mut er) {
            extf.children.push(e);
        }
    }
...
fn parse_extension(r: &mut Cursor<&[u8]>) -> Option<ParsedField> {
    ...
    match marker {
        ...
        _ => {
            ...
            Some(ParsedField {
                name: format!("Unknown Extension (0x{marker:04X})"),
                value: format!("{rem} bytes"),
                raw: d,
                children: vec![],
            })
        }
...
fn parse_psk_extension(r: &mut Cursor<&[u8]>, _: u32) -> ParsedField {
    ...
    for i in 0..tc {
        ...
        if r.read_exact(&mut td).is_err() {
            break;
        }
        let mut tf = ParsedField {
            name: format!("[{i}] Ticket"),
            value: format!("({tl} bytes)"),
            raw: td.clone(),
            children: vec![],
        };
        if let Ok(st) = read_session_ticket(&td) {
            tf.children.push(ParsedField {
                name: "Ticket Type".into(),
                value: format!("0x{:02X}", st.ticket_type),
                raw: vec![],
                children: vec![],
            });
            tf.children.push(ParsedField {
                name: "Lifetime".into(),
                value: format!("{} seconds", st.ticket_lifetime),
                raw: vec![],
                children: vec![],
            });
            tf.children.push(ParsedField {
                name: "Ticket Age Add".into(),
                value: format!("({} bytes)", st.ticket_age_add.len()),
                raw: st.ticket_age_add,
                children: vec![],
            });
            tf.children.push(ParsedField {
                name: "Reserved".into(),
                value: format!("0x{:08X}", st.reversed),
                raw: vec![],
                children: vec![],
            });
            tf.children.push(ParsedField {
                name: "Nonce".into(),
                value: format!("({} bytes): {}", st.nonce.len(), hex::encode(&st.nonce)),
                raw: st.nonce,
                children: vec![],
            });
            tf.children.push(ParsedField {
                name: "Ticket Data".into(),
                value: format!(
                    "({} bytes): {}",
                    st.ticket.len(),
                    truncate_hex(&st.ticket, 32)
                ),
                raw: st.ticket,
                children: vec![],
            });
...
fn parse_ecdhe_extension(r: &mut Cursor<&[u8]>, _: u32) -> ParsedField {
    ...
    for i in 0..kc {
        ...
        let mut sb = [0u8; 2];
        if r.read_exact(&mut sb).is_err() {
            break;
        }
        let ks = u16::from_be_bytes(sb);
        let mut ep = vec![0u8; ks as usize];
        if r.read_exact(&mut ep).is_err() {
            break;
        }
        ...
        field.children.push(ParsedField {
            name: format!("[{i}]"),
            value: format!("Flag={kf}, EC Point ({ks} bytes): {ph}"),
            raw: ep,
            children: vec![],
        });
    }
...
fn parse_server_hello_fields(data: &[u8]) -> Vec<ParsedField> {
    ...
    fields[0].children.push(ParsedField {
        name: "Protocol Version".into(),
        value: format!("0x{:04X}", u16::from_be_bytes(vb)),
        raw: vec![],
        children: vec![],
    });
    if r.read_exact(&mut vb).is_err() {
    ...
    fields[0].children.push(ParsedField {
        name: "Negotiated Cipher Suite".into(),
        value: format!("{} (0x{cs:04X})", cipher_suite_name(cs)),
        raw: vec![],
        children: vec![],
    });
    let mut sr = [0u8; 32];
    if r.read_exact(&mut sr).is_err() {
        return encrypted_fallback(data);
    }
    fields[0].children.push(ParsedField {
        name: "Server Random".into(),
        value: format!("{} (32 bytes)", hex::encode(sr)),
        raw: sr.to_vec(),
        children: vec![],
    });
    let mut el = [0u8; 4];
    if r.read_exact(&mut el).is_err() {
        return fields;
    }
    ...
    let ec = b[0];
    let mut ef = ParsedField {
        name: "Extensions".into(),
        value: format!("({ec}), total {ep} bytes"),
        raw: vec![],
        children: vec![],
    };
    let rem = data.len() - r.position() as usize;
    ...
    let mut er = Cursor::new(es);
    for _ in 0..ec {
        if let Some(e) = parse_server_hello_extension(&mut er) {
            ef.children.push(e);
        }
    }
...
fn parse_server_hello_extension(r: &mut Cursor<&[u8]>) -> Option<ParsedField> {
    ...
    let mut field = ParsedField {
        name: format!("Extension (0x{et:04X})"),
        value: String::new(),
        raw: vec![],
        children: vec![],
    };
    ...
    field.children.push(ParsedField {
        name: "Array Index".into(),
        value: format!("{ai}"),
        raw: vec![],
        children: vec![],
    });
    field.children.push(ParsedField {
        name: "Server EC Public Key".into(),
        value: format!("({kl} bytes): {}", hex::encode(&ep)),
        raw: ep,
        children: vec![],
    });
    ...
    if rem > 0 {
        ...
        field.children.push(ParsedField {
            name: "Extra Data".into(),
            value: format!("({rem} bytes): {}", hex::encode(&ex)),
            raw: ex,
            children: vec![],
        });
    }
    ...
    match read_signature(data) {
        Ok(sig) => vec![ParsedField {
            ...
            children: vec![
                ParsedField {
                    name: "Type".into(),
                    value: format!("0x{:02X}", sig.sig_type),
                    raw: vec![],
                    children: vec![],
                },
                ParsedField {
                    name: "ECDSA Signature".into(),
                    value: format!(
                        "({} bytes): {}",
                        sig.ecdsa_signature.len(),
                        truncate_hex(&sig.ecdsa_signature, 64)
                    ),
                    raw: sig.ecdsa_signature,
                    children: vec![],
                },
            ],
        }],
...
fn parse_new_session_ticket_fields(data: &[u8]) -> Vec<ParsedField> {
    match read_new_session_ticket(data) {
        Ok(nst) => {
            let mut field = ParsedField {
                name: "NewSessionTicket".into(),
                value: String::new(),
                raw: data.to_vec(),
                children: vec![
                    ParsedField {
                        name: "Reversed".into(),
                        value: format!("0x{:02X}", nst.reversed),
                        raw: vec![],
                        children: vec![],
                    },
                    ParsedField {
                        name: "Ticket Count".into(),
                        value: format!("{}", nst.count),
                        raw: vec![],
                        children: vec![],
                    },
                ],
            ...
            for (i, t) in nst.tickets.iter().enumerate() {
                field.children.push(ParsedField {
                    ...
                    children: vec![
                        ParsedField {
                            name: "Type".into(),
                            value: format!("0x{:02X}", t.ticket_type),
                            raw: vec![],
                            children: vec![],
                        },
                        ParsedField {
                            name: "Lifetime".into(),
                            value: format!("{} seconds", t.ticket_lifetime),
                            raw: vec![],
                            children: vec![],
                        },
                        ParsedField {
                            name: "Ticket Age Add".into(),
                            value: format!("({} bytes)", t.ticket_age_add.len()),
                            raw: t.ticket_age_add.clone(),
                            children: vec![],
                        },
                        ParsedField {
                            name: "Reserved".into(),
                            value: format!("0x{:08X}", t.reversed),
                            raw: vec![],
                            children: vec![],
                        },
                        ParsedField {
                            name: "Nonce".into(),
                            value: format!("({} bytes): {}", t.nonce.len(), hex::encode(&t.nonce)),
                            raw: t.nonce.clone(),
                            children: vec![],
                        },
                        ParsedField {
...
fn parse_server_finish_fields(data: &[u8]) -> Vec<ParsedField> {
    match read_server_finish(data) {
        Ok(sf) => vec![ParsedField {
            name: "ServerFinish".into(),
            value: String::new(),
            raw: data.to_vec(),
            children: vec![
                ParsedField {
                    name: "Flag".into(),
                    value: format!("0x{:02X}", sf.reversed),
                    raw: vec![],
                    children: vec![],
                },
                ParsedField {
                    name: "Verify Data".into(),
                    value: format!("({} bytes): {}", sf.data.len(), truncate_hex(&sf.data, 64)),
                    raw: sf.data,
                    children: vec![],
                },
            ],
...
fn parse_psk_extensions_fields(data: &[u8]) -> Vec<ParsedField> {
    let mut fields = vec![ParsedField {
        name: "PSK Extensions (0-RTT)".into(),
        value: String::new(),
        raw: data.to_vec(),
        children: vec![],
    }];
    ...
    fields[0].children.push(ParsedField {
        name: "Total Length".into(),
        value: format!("{}", u32::from_be_bytes(buf)),
        raw: vec![],
        children: vec![],
    });
    let mut b = [0u8; 1];
    ...
    if rem.len() >= 15 {
        ...
        fields[0].children.push(ParsedField {
            name: "Extension Length".into(),
            value: format!("{ext_len}"),
            raw: vec![],
            children: vec![],
        });
        fields[0].children.push(ParsedField {
            name: "Extension Flag".into(),
            value: format!("0x{:02X}", b[0]),
            raw: vec![],
            children: vec![],
        });
        fields[0].children.push(ParsedField {
            ...
            value: format!("{}", r.position()),
            raw: vec![],
            children: vec![],
        });
        fields[0].children.push(ParsedField {
            name: "Marker".into(),
            value: format!("0x{marker:04X}"),
            raw: vec![],
            children: vec![],
        });
        if ts > 0 {
...
fn parse_abort_payload(data: &[u8]) -> Vec<ParsedField> {
    if data.len() >= 5 {
        ...
        if tl as usize == data.len() - 4 {
            return vec![ParsedField {
                name: "Abort (plaintext)".into(),
                value: String::new(),
                raw: data.to_vec(),
                children: vec![
                    ParsedField {
    ...
    vec![ParsedField {
        name: "Abort (encrypted)".into(),
        value: format!("({} bytes)", data.len()),
        raw: data.to_vec(),
        children: vec![ParsedField {
            name: "Data".into(),
            value: truncate_hex(data, 80),
            raw: vec![],
            children: vec![],
        }],
    }]
...
fn parse_application_data(data: &[u8]) -> Vec<ParsedField> {
    ...
    if data.len() >= 16 {
        ...
        if il == data.len() as u32 {
            ...
            field.children.push(ParsedField {
                name: "Data Type".into(),
                value: format!("0x{dt:08X}"),
                raw: vec![],
                children: vec![],
            });
            field.children.push(ParsedField {
                name: "Cmd ID".into(),
                value: format!("0x{ci:08X} ({ci})"),
                raw: vec![],
                children: vec![],
            });
            if data.len() > 16 {
...
fn encrypted_fallback(data: &[u8]) -> Vec<ParsedField> {
    vec![ParsedField {
        name: "Encrypted Payload".into(),
        value: format!("({} bytes): {}", data.len(), truncate_hex(data, 64)),
        raw: data.to_vec(),
        children: vec![],
    }]
}
...
pub fn format_records(records: &[ParsedRecord]) -> String {
    ...
    for (i, rec) in records.iter().enumerate() {
        ...
        let _ = writeln!(
            ...
            "Record #{} [{}] ({} bytes)",
            rec.index,
            record_type_name(rec.record_type),
            rec.length as usize + 5
        );
        let _ = writeln!(
            sb,
            "  Type: {} (0x{:02X})",
...
fn format_field(sb: &mut String, f: &ParsedField, indent: usize) {
    let p = " ".repeat(indent);
    if f.value.is_empty() {
        let _ = writeln!(sb, "{p}{}", f.name);
    } else {
        let _ = writeln!(sb, "{p}{}: {}", f.name, f.value);
    }
    for c in &f.children {
        format_field(sb, c, indent + 2);
    }
}
...
mod tests {
    ...
    fn test_parse_client_hello_ecdhe() {
        ...
        for c in &r.fields[0].children {
            found.insert(c.name.clone());
        }
        for name in [
            "Flag",
            "Protocol Version",
    ...
    fn test_parse_client_hello_psk_zero() {
        let ticket = SessionTicket {
            ...
            reversed: 0x48,
            nonce: vec![0u8; 12],
            ticket: b"test-ticket-data-for-psk".to_vec(),
        };
    ...
    fn test_parse_multiple_records() {
        ...
        data.extend_from_slice(
            &record::create_data_record(0x01, 0x01, b"hello".to_vec()).serialize(),
        );
        let records = parse_records(&data).unwrap();
    ...
    fn test_parse_new_session_ticket() {
        let nst = NewSessionTicket {
            ...
            tickets: vec![SessionTicket {
                ...
                ticket_age_add: vec![0x01, 0x02],
                reversed: 0x48,
                nonce: vec![0u8; 12],
                ticket: b"ticket-data".to_vec(),
            }],
        };
    ...
    fn test_parse_raw_hex() {
        ...
        for (i, c) in hs.as_bytes().chunks(2).enumerate() {
            ...
            sp.push_str(std::str::from_utf8(c).unwrap());
        }
    ...
    fn test_parse_psk_zero_flow() {
        ...
        let t = SessionTicket {
            ...
            reversed: 0x48,
            nonce: vec![
                0x71, 0xae, 0xce, 0xff, 0xd8, 0x3f, 0x29, 0x48, 0x01, 0x02, 0x03, 0x04,
            ],
            ticket: b"psk-access-ticket-data-here".to_vec(),
        };
        all.extend_from_slice(
            &record::create_system_record(client_hello::new_psk_zero_hello(&t).serialize())
                .serialize(),
        );
        let mut ee = vec![0u8; 52];
        ee[..8].copy_from_slice(&[0x47, 0x4c, 0x34, 0x03, 0x71, 0x9e, 0xaa, 0xbb]);
        all.extend_from_slice(&record::create_system_record(ee).serialize());
        let mut ed = vec![0u8; 96];
        ed[..8].copy_from_slice(&[0x98, 0xcd, 0x6e, 0xa0, 0x7c, 0x6b, 0x11, 0x22]);
        all.extend_from_slice(&record::create_raw_data_record(ed).serialize());
        let mut ea = vec![0u8; 40];
        ea[..8].copy_from_slice(&[0x8a, 0xd1, 0xc3, 0x42, 0x9a, 0x30, 0x55, 0x66]);
        all.extend_from_slice(&record::create_abort_record(ea).serialize());
        let records = parse_records(&all).unwrap();
        assert_eq!(records.len(), 4);
        assert_eq!(records[0].record_type, MAGIC_SYSTEM);
        assert_eq!(records[0].fields[0].name, "ClientHello");
        assert_eq!(records[1].record_type, MAGIC_SYSTEM);
        assert_eq!(records[1].fields[0].name, "Encrypted Payload");
        assert_eq!(records[2].record_type, MAGIC_RECORD);
        assert_eq!(records[3].record_type, MAGIC_ABORT);
        assert_eq!(records[3].fields[0].name, "Abort (encrypted)");
        eprintln!("\n{}", format_records(&records));
    }
    ...
    fn test_parse_server_response() {
        let mut all = Vec::new();
        let mut sh = Vec::new();
        sh.extend_from_slice(&[0u8; 4]);
        sh.push(0x02);
        sh.extend_from_slice(&[0xF1, 0x04]);
        sh.extend_from_slice(&[0xC0, 0x2B]);
        sh.extend_from_slice(&[
            0x2b, 0xa6, 0x88, 0x7e, 0x61, 0x5e, 0x27, 0xeb, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
            0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14,
            0x15, 0x16, 0x17, 0x18,
        ]);
        let mut ep = vec![0u8; 65];
        ep[0] = 0x04;
        ep[1..8].copy_from_slice(&[0xfa, 0xe3, 0xdc, 0x03, 0x4a, 0x21, 0xd9]);
        let ei = (2 + 4 + 2 + 65) as u32;
        sh.extend_from_slice(&((4 + ei) as u32).to_be_bytes());
        sh.push(0x01);
        sh.extend_from_slice(&ei.to_be_bytes());
        sh.extend_from_slice(&[0x00, 0x10]);
        sh.extend_from_slice(&[0u8; 4]);
        sh.extend_from_slice(&65u16.to_be_bytes());
        sh.extend_from_slice(&ep);
        let t = (sh.len() - 4) as u32;
        sh[0..4].copy_from_slice(&t.to_be_bytes());
        all.extend_from_slice(&record::create_handshake_record(sh).serialize());
        let mut es = vec![0u8; 80];
        es[..8].copy_from_slice(&[0xb8, 0x79, 0xa1, 0x60, 0xbe, 0x6c, 0x3f, 0x22]);
        all.extend_from_slice(&record::create_handshake_record(es).serialize());
        let mut et = vec![0u8; 120];
        et[..8].copy_from_slice(&[0x1a, 0x6d, 0xc9, 0xdd, 0x6e, 0xf1, 0x88, 0x44]);
        all.extend_from_slice(&record::create_handshake_record(et).serialize());
        let mut ef = vec![0u8; 48];
        ef[..8].copy_from_slice(&[0xb8, 0x79, 0xa1, 0x60, 0xbe, 0x6c, 0x55, 0x99]);
        all.extend_from_slice(&record::create_handshake_record(ef).serialize());
        let records = parse_records(&all).unwrap();
        assert_eq!(records.len(), 4);
        assert_eq!(records[0].fields[0].name, "ServerHello");
        for i in 1..=3 {
            assert_eq!(records[i].fields[0].name, "Encrypted Payload", "record {i}");
        }
        eprintln!("\n{}", format_records(&records));
    }
}
